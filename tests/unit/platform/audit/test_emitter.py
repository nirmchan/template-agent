...
from deep_agent.src.platform.audit.context import bind_audit_context, clear_audit_context
...
class TestEmitAuditEventDisabled:
    def test_noop_when_disabled(self):
        with patch("deep_agent.src.platform.audit.emitter.is_audit_enabled", return_value=False):
            with patch("deep_agent.src.platform.audit.emitter.sys.stdout", new_callable=StringIO) as out:
...
class TestEmitAuditEventEnabled:
    def test_emits_envelope(self):
        ...
        with patch("deep_agent.src.platform.audit.emitter.is_audit_enabled", return_value=True):
            with patch("deep_agent.src.platform.audit.emitter.sys.stdout", new_callable=StringIO) as out:
    ...
    def test_buffers_on_emit_failure(self):
        with patch("deep_agent.src.platform.audit.emitter.is_audit_enabled", return_value=True):
            with patch("deep_agent.src.platform.audit.emitter.sys.stdout") as mock_stdout:
                mock_stdout.write.side_effect = RuntimeError("sink down")
                with patch("deep_agent.src.platform.audit.emitter.enqueue") as mock_enqueue:
