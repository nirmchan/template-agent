...
| Weekly/recurring report | TODO → start_workflow | **Create TODO list first**. Use `start_workflow(skill_name="weekly-report", params={"email": "...", "height_cm": ..., "weight_kg": ...})`. Confirm the schedule is created. |
| Stop weekly reports | signal_workflow | Use `signal_workflow(workflow_id="schedule:schedule-weekly-fitness-report", signal="cancel")`. Confirm cancellation. |
