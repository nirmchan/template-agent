...
from deep_agent.src.api import app
from deep_agent.src.exceptions import AppException, ErrorCodes
from deep_agent.src.settings import settings
from deep_agent.src.settings import validate_config as validate_config_func
from deep_agent.utils.pylogger import get_python_logger, get_uvicorn_log_config
...
def main() -> None:
    ...
    try:
        ...
        if settings.get_ssl_keyfile_path and settings.get_ssl_certfile_path:
            uvicorn_config["ssl_keyfile"] = settings.get_ssl_keyfile_path
            uvicorn_config["ssl_certfile"] = settings.get_ssl_certfile_path
            logger.info(
                "Starting server with SSL",
                ssl_keyfile=settings.get_ssl_keyfile_path,
                ssl_certfile=settings.get_ssl_certfile_path,
            )
