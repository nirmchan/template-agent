...
10    Developer Experience (Deep Agent CLI)  UX Polish (Accessibility)
...
- MR-01: Add aegra-cli to dependencies
- MR-02: Add langgraph-sdk to dependencies
- MR-03: Create deep_agent/aegra/ directory structure
- MR-04: Add .env.example for aegra
- MR-05: Create docker-compose.yml
- MR-06: Add Makefile targets
- MR-07: Update .gitignore
- MR-08: Add aegra docs to README
- MR-09: Define LangGraph State schema
- MR-10: Create state converters
- MR-11: Wrap AgentManager in node
- MR-12: Add skill invocation node
- MR-13: Add subagent orchestration node
- MR-14: Create graph builder
- MR-15: Add checkpointing config

#### Week 2: Core Integration (MRs 16-28) ✅

- MR-16: Implement state serialization
- MR-17: Add error handling to graph
- MR-18: Create graph export function
- MR-19: Create aegra.json configuration
- MR-20: Configure Redis connection
- MR-21: Configure Postgres checkpoints
- MR-22: Set up auth configuration
- MR-23: Configure OpenTelemetry tracing
- MR-24: Add Langfuse integration
- MR-25: Configure worker pool settings
- MR-26: Add health check endpoints
- MR-27: Add unit tests for converters
- MR-28: Add unit tests for graph nodes

#### Week 3: Testing & Deployment (MRs 29-42) ✅

- MR-29: Add BMI skill integration test
- MR-30: Add email skill integration test
- MR-31: Add subagent flow integration test
- MR-32: Add end-to-end test with aegra
- MR-33: Add load test script
- MR-34: Add test fixtures
- MR-35: Update Containerfile
- MR-36: Update K8s manifests
- MR-37: Add deployment scripts
- MR-38: Add Redis to dev compose
- MR-39: Add Langfuse to dev compose
- MR-40: Add Jaeger/OTEL to dev compose
- MR-41: Create unified dev stack with make dev
- MR-42: Add performance benchmarks
...
- MR-43: Add tenacity dependency
- MR-44: Create error_handling.py module (retry decorators, circuit breaker, classify_error, with_fallback)
- MR-45: Add retry decorators to LLM calls (llm.py — @llm_retry, LLMError wrapping)
- MR-46: Add retry to skill invocations (aegra/nodes.py — tenacity-based with_retry replacing hand-rolled)
- MR-47: Add retry to subagent calls (subagent_retry decorator, SubAgentError wrapping)
- MR-48: Implement circuit breaker pattern (Redis-backed with in-memory fallback, TTL auto-expiry)
- MR-49: Add graceful degradation (classify_error, with_fallback decorator)
- MR-50: Add type hints to core agent modules (llm.py, factory.py, manager.py)
- MR-51: Add type hints to skills/
- MR-52: Add type hints to subagents/ and infrastructure (mcp.py, subagents.py)
- MR-53: Add pyright to pre-commit
- MR-54: Tighten mypy config (disallow_untyped_defs, disallow_incomplete_defs, check_untyped_defs, warn_return_any)

**Additional deliverables (beyond original plan):**

- Expanded exception hierarchy: TransientError, LLMError, LLMTimeoutError, MCPError, MCPTimeoutError, SubAgentError, ConfigurationError, RateLimitError, AuthenticationError (with is_retryable property)
- 11 structured error codes (E_001–E_011) with HTTP status mapping
- Redis as explicit dependency (redis>=5.0.0) — promoted from lazy import
- Circuit breaker Redis persistence with HSET/HGETALL + TTL auto-expiry (3x reset_timeout, min 300s)
- 43 unit tests for error_handling.py (classify_error, with_fallback, CircuitBreaker in-memory + Redis-mocked)
...
- MR-01: Upgrade `@langchain/langgraph-sdk` to `^1.9.2`
- MR-02: Add Redux Toolkit, store, hooks, `<Provider>`
- MR-03: Add structured error types (ErrorCode enum, APIError, ErrorHandler)
- MR-04: Add vitest + Testing Library + jsdom

#### State Management (MRs 5-8) ✅

- MR-05: Chats Redux slice — CRUD, streaming state, tool result merging
- MR-06: User settings slice — theme, memory, debug mode, localStorage persistence
- MR-07: Authenticated fetch wrapper — credentials, 401/429 handling
- MR-08: Migrate App, AppLayout, HomePage, ChatPage from ChatContext to Redux

#### BFF Proxy Layer (MRs 9-12) ✅

- MR-09: BFF proxy router with Bearer token auth, SSE pipeline, abort detection
- MR-10: Token refresh in proxy — session-based, pre-forward validation
- MR-11: One-time token endpoint + feedback proxy + agent health check
- MR-12: Redis utility (ioredis) with graceful fallback
...
- PF-01: Add PatternFly 6 dependencies (2h)
- PF-02: Migrate AppLayout to PatternFly Page (3h)
- PF-03: Migrate UI primitives to PatternFly (3h)
- PF-04: Add PatternFly dialogs and alerts (2h)
- PF-05: Update theme system for PatternFly (2h)

**Additional deliverables:**

- `@patternfly/chatbot` dependency added (not yet imported in source)
- `useThemeSync` applies both Tailwind `dark` class and `.pf-v6-theme-dark` on `<html>`
- PatternFly global CSS (`patternfly.css`, `patternfly-addons.css`) loaded in `main.tsx`
- Removed all `@radix-ui/`* dependencies (scroll-area, select, slot, tabs, tooltip)
- Removed `class-variance-authority` — no longer needed
- Deleted 8 Radix/Tailwind wrappers in `components/ui/` (button, badge, card, scroll-area, input, select, tabs, textarea)
- Migrated Button to PF `Button` in ChatPage, Sidebar, ErrorBoundary, ChatErrorBoundary
- Migrated Card/ScrollArea to PF `Card`/`CardBody` in ActivityTimeline
- Migrated Badge to PF `Label` in ChatMessagesView
- Added PF `Modal` for delete-chat confirmation in Sidebar
- Added PF `Alert` + `ExpandableSection` in ErrorBoundary
- Added PF `Alert` (warning variant) in ChatErrorBoundary
- 48 npm packages removed from lockfile
...
- MR-13: Create global streaming manager (4h)
- MR-14: Add SSE chunk processor (3h)
- MR-15: Add `useStreamingAPI` hook (3h)
- MR-16: Wire streaming to BFF proxy (2h)
- MR-17: Add stream cancellation (2h)

#### Chat UI Components (MRs 18-24)

- MR-18: Rebuild ChatMessagesView with Redux (3h)
- MR-19: Add tool call/response rendering (3h)
- MR-20: Add markdown rendering with code highlighting (3h)
- MR-21: Rebuild InputForm with keyboard shortcuts (2h)
- MR-22: Add auto-scroll hook (2h)
- MR-23: Add connection status indicator (2h)
- MR-24: Add toast notification system (2h)

#### Sidebar & Thread Management (MRs 25-30)

- MR-25: Rebuild sidebar with Redux (3h)
- MR-26: Add thread CRUD via BFF proxy (3h)
- MR-27: Add conversation search/filter (2h)
- MR-28: Add delete all conversations (2h)
- MR-29: Rebuild welcome screen (2h)
- MR-30: Add responsive sidebar (2h)

**Additional deliverables (beyond original plan):**

- `StreamEventRenderer` component for structured tool call / intermediate event display
- `ActivityTimeline` component for step-by-step agent activity view
- `ChatErrorBoundary` — chat-level error boundary (originally Phase 5 MR-57)
- `ErrorBoundary` — global error boundary (originally Phase 5 MR-58)
- `RedHatLogo` component with Red Hat branding
- `useRefreshableToken` hook for BFF token refresh
- Auto-send prompt cards from WelcomeScreen
- No-response retry UI
...
- MR-31: Define deep-agent types — `SubAgentStatus`, `SubAgentInfo`, `ToolCallWithContent`, `isSubAgentToolCall()` type guard, `extractSubAgentName()` utility (`types/deep-agent.ts`)
- MR-32: Add sub-agent indicator component — PF `Card` with `Label` status badge, three visual states (delegating/complete/error), expandable args + result view (`SubAgentIndicator.tsx`)
- MR-33: Add sub-agent event processing — BFF `rewriteSubAgentName()` mirrors Python `converter.py` `task`→`subagent_type` rewrite; Redux `StreamingState.activeSubAgent` tracking in `useStreamingAPI`
- MR-34: Add sub-agent status in sidebar — active chat shows blue PF `Label` with sub-agent name and animated spinner during delegation

#### Interrupt / HITL Handler (MRs 35-38) — ✅ COMPLETE

Completed: 2026-05-13

- MR-35: Add interrupt detection in streaming — `interrupt` SSEChunk type, `pendingInterrupt` in `StreamingState`, `onInterrupt` callback in `StreamingManager`, BFF thread state check post-stream
- MR-36: Add ToolApprovalInterrupt component — `InterruptBanner` with approve/reject buttons for tool approval interrupts (PF `Alert variant="warning"`)
- MR-37: Add generic interrupt dialog — Same `InterruptBanner` handles text-response interrupts (PF `Alert variant="info"` + `TextInput`)
- MR-38: Add interrupt resume flow in BFF — `resume` flag in `StreamRequestBody`, BFF uses `command: { resume }` instead of `input: { messages }` when set

#### File Artifacts & Task Progress (MRs 39-42) — ✅ COMPLETE

Completed: 2026-05-13

- MR-39: Add file artifacts viewer — `ArtifactViewer` with syntax detection (`detectArtifactKind`: code/json/markdown/text), copy button, ReactMarkdown rendering; wired into `AIMessageRenderer` for rich tool results
- MR-40: Add todo/task progress stepper — `TaskProgressStepper` derives steps from tool calls in messages, shows PF `Label` chain with status icons (running/complete), mounted above chat when tool calls present
- MR-41: Add tasks/files sidebar panel — `TasksSidebar` extracts tool call entries from messages, shows completion count, result previews, artifact kind labels; visible on lg+ screens when tool calls present
- MR-42: Add debug mode toggle — `DebugToggle` (Bug icon in masthead), `DebugPanel` (message counts, streaming state JSON); `debugMode` already existed in `userSettings` Redux slice
...
- MR-43: Add memories API service — `personalization` Redux slice with `addMemory`, `removeMemory`, `clearMemories`; localStorage persistence (`template-ui-personalization`); BFF forwards `memories[]` in stream request `config.configurable`
- MR-44: Add BFF proxy routes for memories — `proxy.router.ts` updated: `StreamRequestBody.memories` field extracted, forwarded as `config.configurable.user_memories` in LangGraph run body
- MR-45: Add MemoryListView component — `MemoryList.tsx` with add/delete/clear, info callout, empty state, Brain icon
- MR-46: Add memory toggle — `memoryEnabled` already in `userSettings` slice; memories sent only when non-empty
- MR-47: Add memories to settings page — "Memories" tab in `SettingsPage.tsx`

#### User Rules (MRs 48-50) — ✅ COMPLETE

- MR-48: Add rules API service — `personalization` Redux slice with `addRule`, `updateRule`, `toggleRule`, `removeRule`, `clearRules`; `selectActiveRules` selector; BFF forwards `rules[]` via `config.configurable.user_rules`
- MR-49: Add RulesEditor component — `RulesEditor.tsx` with add/toggle/delete/clear, PF `Switch` for per-rule enable/disable, amber info callout
- MR-50: Add rules to settings page — "Custom Rules" tab in `SettingsPage.tsx`

#### Settings & Themes (MRs 51-54) — ✅ COMPLETE

- MR-51: Create settings page — `SettingsPage.tsx` with 4-tab layout (Profile, Memories, Custom Rules, Appearance); responsive nav + content; back-to-home button; route `/settings` in `App.tsx`
- MR-52: Add theme selector — `AppearanceSettings.tsx` with visual card picker (Light/Dark) using `setTheme` dispatch; Interface Density placeholder
- MR-53: Add user profile section — `ProfileSection.tsx` shows avatar initial, display name, username, email, SSO badge from `window.USER_DATA`
- MR-54: Add settings link in sidebar footer — `Sidebar.tsx` updated with `Settings` icon link via `useNavigate('/settings')`

#### Backend: Personalization Module — ✅ COMPLETE

- `deep_agent/src/personalization/models.py` — Pydantic `Memory` + `Rule` models with UUID, user_id, timestamps
- `deep_agent/src/personalization/repository.py` — Async Postgres CRUD (`PersonalizationRepository`) using `psycopg` for `user_memories` + `user_rules` tables with lazy table creation
- `deep_agent/src/personalization/injector.py` — `inject_personalization()` appends memories + rules sections to system prompt
- `deep_agent/aegra/graph.py` — Modified to read user personalization from Postgres at graph creation time and inject into system prompt (graceful fallback if tables don't exist)
- `config/migrations/001_personalization.sql` — DDL for `user_memories` + `user_rules` tables with indexes
